  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg" onClick={(e) => e.target === e.currentTarget && onClose()}>
        <motion.div initial={{ scale: 0.95, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 30 }} className="bg-card border border-border w-full max-w-5xl rounded-3xl shadow-2xl shadow-primary/10 flex flex-col max-h-[85vh] overflow-hidden">
          <div className="p-6 border-b border-border flex justify-between items-center bg-card">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 flex items-center justify-center"><Wand2 size={24} className="text-primary" /></div>
              <div>
                <h2 className="text-2xl font-bold text-white tracking-tight">AI Content Studio</h2>
                <div className="flex items-center gap-2 text-sm text-neutral-400">Target: <span className="text-white font-semibold bg-neutral-800 px-2 py-0.5 rounded-md max-w-[300px] truncate">{trend.topic}</span></div>
              </div>
            </div>
            <button onClick={onClose} className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-white hover:bg-neutral-800 transition-all"><X size={20} /></button>
          </div>

          <div className="flex flex-1 overflow-hidden">
            <div className="w-72 bg-card border-r border-border p-4 flex flex-col gap-2">
               {tabs.map(tab => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl text-left transition-all w-full
            ...
            <div className="flex-1 p-8 bg-card relative overflow-y-auto custom-scrollbar">
                {!result && !loading && !tabs.find(t => t.id === activeTab)?.disabled && (
                    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="h-full flex flex-col justify-center items-center text-center max-w-md mx-auto">
                        <div className="w-20 h-20 bg-neutral-900 rounded-3xl flex items-center justify-center mb-6"><Wand2 size={40} className="text-primary" /></div>
                        <h3 className="text-2xl font-bold text-white mb-2">AI Magic Ready</h3>
                        <p className="text-neutral-400 mb-8">Generate content for "{trend.topic}" in {isArabic(trend.fullTitle || trend.topic) ? 'Arabic' : 'English'}.</p>
                        
                        <button onClick={handleGenerate} className="px-8 py-4 bg-white text-black rounded-2xl font-bold hover:scale-105 transition-transform shadow-xl hover:shadow-white/10 flex items-center gap-2">
                 ...
                 {!result && !loading && tabs.find(t => t.id === activeTab)?.disabled && (
                    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="h-full flex flex-col justify-center items-center text-center max-w-md mx-auto">
                        <div className="w-20 h-20 bg-neutral-900 rounded-3xl flex items-center justify-center mb-6"><Lock size={40} className="text-primary" /></div>
                        <h3 className="text-2xl font-bold text-white mb-2">Upgrade to Unlock</h3>
                        <p className="text-neutral-400 mb-8">This feature is available on our Pro and Yearly plans. Upgrade your account to generate video scripts and deploy AI agents.</p>
                        
                        <button onClick={() => router.push('/dashboard/subscriptions')} className="px-8 py-4 bg-primary text-primary-foreground rounded-2xl font-bold hover:scale-105 transition-transform shadow-xl shadow-primary/20 flex items-center gap-2">
                ...
                {result && !loading && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        <div className="bg-neutral-900/50 border border-border rounded-2xl p-6">
                             {renderResult()}
                        </div>
